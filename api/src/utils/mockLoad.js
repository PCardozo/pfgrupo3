...
async function loadAnimals(){
    for (let i = 0; i < mockAnimals.length; i++) {
        ...
        let queryAnimalType =  await Animal_type.findOne({
            where : {
                name: mockAnimals[i].animal_typeName
            }
        })
        
        await newanimal.addAnimal_types(queryAnimalType)
    }
}
...
async function loadCarts(){
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        await Product.findByPk(items[i].productId);
        await user.createCart_item({productId:items[i].productId,quantity:items[i].quantity});
    }
}
...
async function loadPetitions(){
    try {
        for (let i = 0; i < mockPetitions.length; i++) {
            let petition = await Adoption_petition.create({topic:mockPetitions[i].topic,description:mockPetitions[i].description, read: mockPetitions[i].read, isImportant: mockPetitions[i].isImportant});
            ...
            await user.addAdoption_petitions(petition);
            await animal.addAdoption_petitions(petition);
        }
    } catch (error) {
        console.log(error)
    }    
...
async function loadAltas(){
    try {
        for (let i = 0; i < mockAltas.length; i++) {
            ...
            await user.addAdoption_alta(alta);
        }
    } catch (error) {
        console.log(error)
    }    
